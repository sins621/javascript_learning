...
function findJokes(jokeType, jokesObject) {
  let filteredJokes = [];
  for (let i = 0; i < jokesObject.length; i++) {
    if (jokesObject[i].jokeType === jokeType) {
      filteredJokes.push(jokesObject[i]);
    }
  }
...
app.get("/filter", (req, res) => {
  ...
  let filteredJokes = findJokes(jokeType, jokes);
  if (filteredJokes.length !== 0) {
    res.send(filteredJokes);
  } else {
    res.status(400).send("We don't have Jokes in that Type");
  }
});
